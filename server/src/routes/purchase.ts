...
purchaseRouter.post('/purchase', async (req: Request, res: Response) => {
    ...
    const sellerPrivateKey = process.env.TRANSFER_ACCOUNT_PRIVATE_KEY;
    if (!sellerPrivateKey) {
    ...
    try {
        ...
        const buyerAccount = Account.fromPrivateKey({ privateKey });
        ...
        const sellerAccount = Account.fromPrivateKey({ privateKey: sellerPrivateKey });
        ...
        const transaction = await aptos.transaction.build.simple({
            ...
            data: {
                ...
                functionArguments: [buyerAccount.accountAddress.toString(), amount],
            },
        });
